...
class SailsCalls {
    ...
    static new = (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            const api$1 = await api.GearApi.create({
    ...
    command = (options) => {
        return new Promise(async (resolve, reject) => {
            const { signerData, contractToCall, serviceName, methodName, callArguments, tokensToSend = 0n, voucherId, gasLimit, callbacks } = options;
    ...
    query = (options) => {
        return new Promise(async (resolve, reject) => {
            const { contractToCall, serviceName, methodName, userAddress = sailsJs.ZERO_ADDRESS, callArguments = [], callbacks } = options;
            let contractSailsInstance;
            if (contractToCall) {
                if (typeof contractToCall === 'string') {
                    const temp = this.sailsInstances[contractToCall];
    ...
    */
    ...
    voucherBalance = (voucherId) => {
        return new Promise(async (resolve) => {
            ...
            const voucherBalanceFormated = Number(BigInt(voucherBalance.toString()) / 1000000000000n);
     ...
     */
    formatContractSignlessData = (signlessData, signlessName) => {
        const temp = {
            encoding: {
