var sailsJs = require('sails-js');
var api = require('@gear-js/api');
var sailsJsParser = require('sails-js-parser');
...
class SailsCalls {
    ...
    lookedEvents;
    constructor(api, parser, newContractsData, accountToSignVouchers) {
        this.gearApi = api;
        ...
        for (const newContractData of newContractsData) {
            ...
            const sailsInstance = new sailsJs.Sails(parser);
    ...
    static decodeAddress = (publicKey) => {
        return api.decodeAddress(publicKey);
    };
    /**
     ...
     */
    sailsInstanceOfContract(contractName) {
        if (contractName) {
    ...
    static create = (data) => {
        return new Promise(async (resolve, reject) => {
            const { newContractsData = [], network = 'ws://localhost:9944', voucherSignerData, gearApi } = data || {};
            let voucherSigner = null;
            if (voucherSignerData) {
                const { sponsorName, sponsorMnemonic } = voucherSignerData;
                try {
                    voucherSigner = await api.GearKeyring.fromMnemonic(sponsorMnemonic, sponsorName);
                }
                catch (e) {
                    const error = {
            ...
            const api$1 = gearApi
                ? gearApi
                : await api.GearApi.create({
                    providerAddress: network
                });
            const sailsParser = await sailsJsParser.SailsIdlParser.new();
            try {
                const sailsInstance = new SailsCalls(api$1, sailsParser, newContractsData, 
                // network,    
                voucherSigner);
     ...
     * });
     */
    static new = SailsCalls.create;
    contractInstanceToCall = (contractToCall) => {
        ...
        if (contractToCall) {
            if (typeof contractToCall === 'string') {
                ...
                contractSailsInstance = new sailsJs.Sails(this.sailsParser);
                try {
     ...
     * });
     */
    subscribeTo = ({ serviceName, eventName, contractToCall, onEventEmit }) => {
        let contractSailsInstance;
        ...
        const functionsNames = this.serviceFunctionNamesFromSailsInstance(contractSailsInstance, serviceName, 'events');
        ...
        const unsubId = utils.generateUUID();
            ...
            try {
                ...
                if (gasLimit) {
                    if (typeof gasLimit === 'object') {
                        await transaction.calculateGas(true, gasLimit.extraGasInCalculatedGasFees);
                    }
                    else {
    ...
    query = (options) => {
        return new Promise(async (resolve, reject) => {
            const { contractToCall, serviceName, methodName, userAddress, callArguments = [], callbacks } = options;
            let contractSailsInstance;
            const result = this.contractInstanceToCall(contractToCall);
            if (result.error == false) {
                contractSailsInstance = result.data;
            ...
            try {
                if (userAddress)
                    queryBuilder.withAddress(userAddress);
    ...
    withAccountToSignVouchers = (sponsorMnemonic, sponsorName) => {
        return new Promise(async (resolve, reject) => {
            try {
                const voucherSigner = await api.GearKeyring.fromMnemonic(sponsorMnemonic, sponsorName);
     ...
     */
    revokeVoucher = (userAddress, voucherId, enableLogs, callbacks) => {
        return new Promise(async (resolve, reject) => {
            ...
            const extrinsic = await this.gearApi
                .voucher
                .revoke(userAddress, voucherId);
            try {
                await this.signVoucherAction(extrinsic, enableLogs ?? false, callbacks);
                resolve();
            }
            catch (e) {
                reject(e);
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const name = nameOfSignlessAccount
                    ...
                    : 'signlessPair';
                const newPair = await api.GearKeyring.create(name);
    ...
    unlockKeyringPair = (pair, password) => {
        return api.GearKeyring.fromJson(pair, password);
    };
    /**
